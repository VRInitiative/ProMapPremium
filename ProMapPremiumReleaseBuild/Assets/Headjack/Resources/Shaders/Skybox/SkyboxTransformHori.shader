Shader "Skybox/Transform Horizontal Cubemap Video" {
	Properties{
		_Tint("Tint Color", Color) = (.5, .5, .5, .5)
		[Gamma] _Exposure("Exposure", Range(0, 8)) = 1.0
		_Rotation("Rotation", Range(0, 360)) = 0
		_Squeeze("Squeeze", Range(0, 1)) = 0.99
...
SubShader {
	...
	v2f fb_vert (appdata_t v, half2 trans, half rot)
	{
		...
		o.texcoord = RotateUVInDegrees(o.texcoord, 90);
	...
	Pass{
		...
		half4 _MainTex_HDR;
		half4 frag(v2f i) : SV_Target{ return skybox_frag(i,_MainTex, _MainTex_HDR); }
		v2f vert(appdata_t v) { return fb_vert(v, half2(0, 2), 0); }
		ENDCG 
	}
	// left
	Pass{
	...
	Pass{
		...
		half4 _MainTex_HDR;
		half4 frag(v2f i) : SV_Target{ return skybox_frag(i,_MainTex, _MainTex_HDR); }
		v2f vert(appdata_t v) { return fb_vert(v, half2(1, 2), 0); }
		ENDCG 
	}
	// top
	Pass{
