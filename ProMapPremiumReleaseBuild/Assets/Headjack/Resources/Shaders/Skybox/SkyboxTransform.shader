Shader "Skybox/Transform Cubemap Video" {
	Properties{
...
SubShader {
	...
	v2f fb_vert (appdata_t v, half2 trans, half rot)
	{
		...
		float2 mid = float2(_MainTex_ST.z + (0.166667 + trans.x * 0.333333) * _MainTex_ST.x, _MainTex_ST.w + (0.25 + trans.y * 0.5) * _MainTex_ST.y);
		float2 scale = float2(_Squeeze * 0.33333 * _MainTex_ST.x, _Squeeze * 0.5 * _MainTex_ST.y);
	...
	Pass{
		...
		half4 _MainTex_HDR;
		half4 frag(v2f i) : SV_Target{ return skybox_frag(i,_MainTex, _MainTex_HDR); }
		v2f vert(appdata_t v) { return fb_vert(v, half2(0, 0), 0); }
		ENDCG 
	}
	Pass{
		CGPROGRAM
		...
		half4 _MainTex_HDR;
		half4 frag(v2f i) : SV_Target{ return skybox_frag(i,_MainTex, _MainTex_HDR); }
		v2f vert(appdata_t v) { return fb_vert(v, half2(2, 1), 0); }
		ENDCG
	}
	Pass{
		CGPROGRAM
		...
		half4 _MainTex_HDR;
		half4 frag(v2f i) : SV_Target{ return skybox_frag(i,_MainTex, _MainTex_HDR); }
		v2f vert(appdata_t v) { return fb_vert(v, half2(0, 1), 0); }
		ENDCG 
	}
	Pass{
		CGPROGRAM
		...
		half4 _MainTex_HDR;
		half4 frag(v2f i) : SV_Target{ return skybox_frag(i,_MainTex, _MainTex_HDR); }
		v2f vert(appdata_t v) { return fb_vert(v, half2(2, 0), 0); }
		ENDCG 
	}
	Pass{
		CGPROGRAM
		...
		half4 _MainTex_HDR;
		half4 frag(v2f i) : SV_Target{ return skybox_frag(i,_MainTex, _MainTex_HDR); }
		v2f vert(appdata_t v) { return fb_vert(v, half2(1, 0), 0); }
		ENDCG 
	}
}
}
