	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float2 fade = saturate((_FadeBorders.xy - abs(i.canvasPosition.xy))*(_FadeBorders.xy / (_FadeBorders.zw*_FadeBorders.xy)));
				return col * float4(i.color.rgb, min(fade.x,fade.y)*i.color.a);
			}
			ENDCG
		}
	}
